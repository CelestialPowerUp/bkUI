define(["views/modules/base", "views/webix/baidumap", "views/modules/upload"], function (base, undefined, upload) {
    ...
    var elements = [
        ...
        {view: "text", label: "首单奖励", name: "first_order_reward", placeholder: "首单奖励金额", width: 350, required: true},
        {
            view: "text",
            label: "匹配距离（米）",
            name: "adaption_distance",
            placeholder: "社区店最大匹配距离",
            width: 350,
            required: false
        },
        {view: "text", label: "银行卡号", id: "bank_card", name: "bank_card", placeholder: "这里面填写银行卡号", width: 350},
        {
            view: "label",
            width: 350,
            template: "<span id='img_files_picker' style='text-decoration: underline; color: blueviolet;'>上传图片</span>",
            on: {
                "onAfterRender": function () {
                    upload.$bind_upload("img_files_picker", function (data) {
                        if (data.code === '00000' && data.data != null) {
                            var item = data.data;
                            item.img_id = item.id;
                            $$("attachments_id").parse(item);
                        }
                    });
                }
            }
        },
        {
            ...
            template: function (obj) {
                if (obj && obj.raw_url) {
                    this.value = [obj.id];
                    return "<img src='" + obj.raw_url + "' width='330px' height='198px'>";
                }
                this.value = [];
                return "<img src='http://7xiqd8.com2.z0.glb.qiniucdn.com/Fg_yYLTcb6lsCJaKI9DMIBeD53VF' width='330px' height='198px'>";
            },
            getValue: function () {
                return this.value;
            }
            ...
            cols: [
                {},
                {
                    ...
                    if ($$("form_view").validate()) {
                        ...
                        form_data.attachments_id = $$("attachments_id").config.getValue();
    ...
    var init_data = function () {
        ...
        base.getReq("/v2/api/supplier.json/" + id, function (data) {
            ...
            $$("attachments_id").parse(data.attachments_id[0]);
            var address = {};
