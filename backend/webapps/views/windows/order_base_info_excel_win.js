...
define(["views/modules/base"],function(base){
    ...
    var button_ui = {
        ...
        cols:[
            ...
            {view:"button",label:"导出",width:80,click:function(){
                ...
                base.postReq("excel/order_base_info.json",{start:start,end:end},function(data){
                    window.open(data);
                    base.$msg.info("excel导出成功");
                });
            }},
            {view:"button",label:"取消",width:80,click:function(){
                webix.$$("order_base_info_excel_win").close();
            }}
        ]
    };
    ...
    var win_ui = {
        view:"window",
        modal:true,
        id:"order_base_info_excel_win",
        position:"center",
        head:{
            view:"toolbar",height:40, cols:[
                {view:"label", label: "订单基本信息" },
                { view:"button", label: 'X', width: 35, align: 'right', click:"$$('order_base_info_excel_win').close();"}
            ]},
        body:{
            paddingX:15,
            paddingY:15,
